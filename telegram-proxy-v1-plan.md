...
- **Мульти-источники passthrough** (бэкфилл старых анонсов форвардами в
  дополнительную группу; на обоих контурах — постоянно): `telegram.sources` =
  `chatId[:topicId]` через запятую, складывается с парой `telegram.source.*`;
  медиа с caption тоже уходит в passthrough (модель видит весь апдейт). Туда же
  постоянно выставлен `SCHEDULER_INBOX_PARALLELISM=1` — строгая
  последовательность флоу. Откат обоих — очисткой соответствующих ENV.
