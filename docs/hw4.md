...
ДЗ4 требует: (1) небольшой набор данных; (2) векторный поиск по нему;
(3) хранение связей между объектами; (4) гибридный поиск (по смыслу + по связям);
(5) проверку работы на примере запроса — «код + пример запроса/ответа».
