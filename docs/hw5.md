...
ДЗ5 требует: задачу из 3–5 шагов; последовательность выполнения; одно ветвление
или проверку; подключение инструмента или памяти; зафиксированную логику.
Формат сдачи — код/схема + пример выполнения.
...
Ветвления ДЗ — это шаг 5 (четыре исхода из одного финала) + лимит циклов.
...
## Пример выполнения (реальный прогон, 2026-09-14)
...
Интересная деталь прогона: тул-чек на шаге 3 нашёл кандидата с similarity 0.875
(серая зона) — модель честно вернула NEEDS_REVIEW/POSSIBLE_DUPLICATE и НЕ создала
событие; финальная дубль-проверка оркестратора по полному представлению дала
